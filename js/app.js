...
// ================= 11. سيستم قطيع الأمهات والمفرخ الذكي =================
let currentGoodEggs = 0;

// حساب الإحصائيات الفورية والمشاكل (Troubleshooting Engine)
window.calcBreederStats = () => {
    const total = parseInt(document.getElementById('brTotalEggs')?.value) || 0;
    const bad = parseInt(document.getElementById('brBadEggs')?.value) || 0;
    const good = total - bad;
    
    const goodDisplay = document.getElementById('brGoodEggsDisplay');
    const rateDisplay = document.getElementById('brRateDisplay');
    const analysis = document.getElementById('brAnalysis');
    
    // حساب سعة المكنة ديناميكياً من الإعدادات
    const shelves = parseInt(document.getElementById('setIncShelves')?.value) || 5;
    const cap = parseInt(document.getElementById('setIncShelfCapacity')?.value) || 100;
    if(document.getElementById('incTotalCapacityDisplay')) {
        document.getElementById('incTotalCapacityDisplay').innerText = (shelves * cap);
    }

    if(goodDisplay) goodDisplay.innerText = good < 0 ? 0 : good;

    if(total > 0) {
        const rate = ((good / total) * 100).toFixed(1);
        const badRate = ((bad / total) * 100).toFixed(1);
        rateDisplay.innerText = `${rate}%`;

        // مقارنة النسب بالمعايير العالمية
        if(rate >= 85) {
            rateDisplay.style.color = 'var(--success)';
            analysis.innerHTML = `<span style="color:var(--success)">✅ معدل الفرز طبيعي وممتاز.<br> (المعدل العالمي للبيض الصالح 85% - 95%)</span>`;
        } else {
            rateDisplay.style.color = 'var(--danger)';
            analysis.innerHTML = `<span style="color:var(--danger)">🚨 تحذير: نسبة البيض المستبعد (${badRate}%) غير طبيعية ومقلقة!<br>
            <strong style="color:#000;">💡 أسباب محتملة للتدخل السريع:</strong><br>
            1. نقص الكالسيوم أو الفسفور في العليقة.<br>
            2. كثافة عالية للطيور داخل الأقفاص أدت لدهس البيض.<br>
            3. إجهاد حراري أو خلل في ساعات الإضاءة.<br>
            4. انخفاض جودة القشرة بسبب كبر عمر الأمهات.</span>`;
        }
    } else {
        rateDisplay.innerText = '0%';
        analysis.innerHTML = '';
    }
};

// حفظ بيانات الفرز اليومي في الداتا بيز
window.saveBreederLog = async () => {
    const mothers = parseInt(document.getElementById('brMothers')?.value) || 0;
    const total = parseInt(document.getElementById('brTotalEggs')?.value) || 0;
    const bad = parseInt(document.getElementById('brBadEggs')?.value) || 0;
    if(!total) return showToast("برجاء إدخال إجمالي البيض", true);

    const good = total - bad;
    if(good < 0) return showToast("البيض المستبعد لا يمكن أن يكون أكبر من الكلي!", true);

    await push(ref(db, 'breedersLogs'), {
        date: new Date().toISOString().split('T')[0],
        mothers, total, bad, good,
        timestamp: Date.now()
    });

    // إضافة البيض الصالح لرصيد المفرخ
    await set(ref(db, 'inventory/goodEggsStock'), currentGoodEggs + good);
    
    // تحديث رقم الأمهات في الإعدادات لتذكره غداً
    if(mothers > 0) await update(ref(db, 'settings'), { currentMothers: mothers });

    closeModal('modalBreederDaily');
    showToast("تم حفظ إنتاج الأمهات وتحديث الرصيد 🥚");
};

// النقل الآلي من رصيد الأمهات لدفعة المفرخ
window.transferToIncubator = (qty) => {
    openModal('modalBatch');
    setTimeout(async () => {
        document.getElementById('bEggs').value = qty;
        await set(ref(db, 'inventory/goodEggsStock'), currentGoodEggs - qty);
        showToast("تم سحب البيض من الرصيد وتجهيزه كدفعة جديدة بالمفرخ!");
    }, 400);
};

// الاستماع المستمر لتراكم البيض وإصدار إشعار المكنة
onValue(ref(db, "inventory/goodEggsStock"), (snapshot) => {
    currentGoodEggs = snapshot.exists() ? snapshot.val() : 0;
    const display = document.getElementById('breederTotalGood');
    if(display) display.innerText = currentGoodEggs;

    const shelves = parseInt(document.getElementById('setIncShelves')?.value) || 5;
    const cap = parseInt(document.getElementById('setIncShelfCapacity')?.value) || 100;
    const incubatorThreshold = shelves * cap;

    const alertsContainer = document.getElementById('breederAlertsContainer');
    if(alertsContainer) {
        if(currentGoodEggs >= incubatorThreshold && incubatorThreshold > 0) {
            alertsContainer.innerHTML = `
                <div style="background: rgba(14, 165, 233, 0.1); border-right: 4px solid var(--info); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    <h4 style="margin: 0 0 10px 0; color: var(--info);">🔔 اكتملت سعة مكنة التفريخ!</h4>
                    <p style="margin: 0 0 10px 0; color: var(--text-primary); font-size: 14px;">تراكم لديك <strong>${currentGoodEggs}</strong> بيضة صالحة، وسعة المكنة المطلوبة هي <strong>${incubatorThreshold}</strong>. حان وقت التشغيل!</p>
                    <button class="btn btn-info btn-sm" onclick="transferToIncubator(${incubatorThreshold})" style="width: auto; padding: 10px 15px;">إدخال البيض للمفرخ الآن 📥</button>
                </div>
            `;
        } else {
            alertsContainer.innerHTML = '';
...
// استدعاء آخر عدد تم تسجيله للأمهات
onValue(ref(db, "settings/currentMothers"), (snapshot) => {
    if(snapshot.exists()) {
        const m = snapshot.val();
        if(document.getElementById('breederTotalMothers')) document.getElementById('breederTotalMothers').innerText = m;
        if(document.getElementById('brMothers')) document.getElementById('brMothers').value = m;
    }
});

// رسم سجل الفرز اليومي للأمهات
onValue(ref(db, "breedersLogs"), (snapshot) => {
    const list = document.getElementById('breederList');
    if(!list) return;
    if(!snapshot.exists()) {
        list.innerHTML = '<div style="text-align:center; color:var(--text-secondary); padding: 20px;">لا يوجد سجلات إنتاج للأمهات بعد.</div>';
        return;
    }
    let html = '';
    const logs = snapshot.val();
    Object.keys(logs).sort((a,b) => logs[b].timestamp - logs[a].timestamp).forEach(k => {
        const l = logs[k];
        const rate = ((l.good/l.total)*100).toFixed(1);
        const color = rate >= 85 ? 'var(--success)' : 'var(--danger)';
        html += `
        <div class="transaction-item" style="border-right: 4px solid ${color}; padding-right: 10px; margin-bottom: 8px;">
            <div>
                <b>تاريخ: ${l.date}</b><br>
                <span style="font-size:12px; color:var(--text-secondary);">إنتاج كلي: ${l.total} | صالح: ${l.good} | مستبعد: ${l.bad}</span>
            </div>
            <div style="font-weight:900; color:${color}; text-align:center;" dir="ltr">
                ${rate}%<br>
                <span style="font-size:10px; color:var(--text-secondary);">صلاحية</span>
            </div>
        </div>`;
    });
    list.innerHTML = html;
